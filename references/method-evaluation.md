...
The four cases are regression seeds, not coverage of a whole product. Add a case only for a demonstrated reusable failure. Project-specific browser/Writer/research requirements belong to their project's harness and frozen cases. Live host delivery uses the matching adapter acceptance section; this local helper does not prove Cursor browser paths, Codex wakeup, or that a host pointer stayed unload-on-`skip`. Cross-host usefulness is claimed only after Cursor and Codex both have executed kernel transcripts.
